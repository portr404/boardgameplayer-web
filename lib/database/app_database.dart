...
@DriftDatabase(tables: <Type>[Players])
class AppDatabase extends _$AppDatabase {
  AppDatabase() : super(driftDatabase(name: 'board_game_manager'));

  @override
  int get schemaVersion => 1;

  Stream<List<Player>> watchPlayers() {
    return (select(players)..orderBy(<OrderClauseGenerator<Players>>[
  ...
  Future<int> addPlayer(String name) {
    return into(players).insert(PlayersCompanion.insert(name: name.trim()));
  }

  Future<void> deletePlayer(int playerId) {
    return (delete(
      players,
    )..where((Players table) => table.id.equals(playerId))).go();
  }
